...
void Trie::insert(string new_word, string element_name, int id, int position)
{
    ...
    // percorre a arvore adicionando novos nodePoints (caso necessário) para cada letra da palavra
    for(unsigned int i = 0; i < wcslen(w); i++)
    {
        ...
        {
            current->child[letter] = new nodePoint();
        }
	...
	/* caso a palavra está entrando na trie pela primeira vez, no ultimo nodePoint é criado uma lista para guardar ponteiros para 
	   elementos que possuam esta palavra no nome */
    if(current->is_end == false)
    {
        current->is_end = true; // informa que o nodePoint é o ultimo nodePoint daquela palavra
...
void Trie::interator_trie(nodePoint *current, bool is_prefix)
{
    // comando basico
	...
	// comando recursivo: utiliza um for, pois cada nodePoint possui um hash map de filhos. O for percorre todos os filhos
...
// função que pega o prefixo, caminha até achar o ultimo nodePoint do prefixo (acha a subarvore)
...
void Trie::insert_machine(string path){
     ...
     if (myReadFile.is_open())
     {
		 ...
		 cout << "Error in loading base file!" << endl;
	 }
